...
```bash
pnpm exec repomix . --include "docs/architecture/**/*.md,packages/backend/src/services/workflow*.ts,packages/backend/src/http/traceRoutes.ts" --output artifacts/repomix/local-starter.xml --style xml --compress
```
