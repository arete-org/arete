...
function generateJWT() {
    if (!GITHUB_APP_ID || !GITHUB_APP_PRIVATE_KEY_PATH) {
        throw new Error(
            'GITHUB_APP_ID and GITHUB_APP_PRIVATE_KEY_PATH environment variables are required'
        );
    }
    ...
    return require('jsonwebtoken').sign(payload, privateKey, {
        algorithm: 'RS256',
    });
}
...
async function getInstallationToken() {
    ...
    if (!installationsResponse.ok) {
        throw new Error(
            `Failed to get installations: ${installationsResponse.status} ${installationsResponse.statusText}`
        );
    }

    const installations = await installationsResponse.json();
    const installation = installations.find(
        (inst) => inst.account.login === REPO_OWNER
    );

    if (!installation) {
        throw new Error(
            `GitHub App not installed on ${REPO_OWNER}/${REPO_NAME}`
        );
    }
    ...
    if (!tokenResponse.ok) {
        throw new Error(
            `Failed to get installation token: ${tokenResponse.status} ${tokenResponse.statusText}`
        );
    }
...
async function fetchDiscussions() {
    ...
    return discussions.filter(
        (discussion) => discussion.category?.name === CATEGORY_NAME
    );
}
...
async function fetchCommentsCount(discussionNumber) {
    try {
        ...
        if (!response.ok) {
            console.warn(
                `Could not fetch comments for discussion ${discussionNumber}`
            );
            return 0;
        }
        ...
        console.warn(
            `Error fetching comments for discussion ${discussionNumber}:`,
            error.message
        );
        return 0;
    }
...
async function writeBlogPost(discussion) {
    try {
        ...
        const postFilePath = path.join(
            BLOG_POSTS_DIR,
            `${discussion.number}.json`
        );
        await fs.writeFile(postFilePath, JSON.stringify(postObject, null, 2));

        console.log(
            `✅ Synced blog post: ${discussion.title} (#${discussion.number})`
        );
        return postObject;
    } catch (error) {
        console.error(
            `❌ Error writing blog post ${discussion.number}:`,
            error
        );
        throw error;
    }
    ...
    try {
        ...
        for (const discussion of discussions) {
            try {
                ...
                console.error(
                    `Failed to process discussion ${discussion.number}:`,
                    error.message
                );
            }
        }
