...
import {
    ...
    shortHash,
} from '../src/shared/pseudonymization';
...
test('hmacId hashes consistently per namespace and secret', () => {
    ...
    assert.equal(
        userHash.length,
        64,
        'HMAC digest should be 64 hex characters'
    );
    assert.equal(
        userHash,
        userHashRepeat,
    ...
    assert.notEqual(
        userHash,
        guildHash,
        'Namespace prevents collisions across ID types'
    );
    assert.notEqual(
        userHash,
        userHashOtherSecret,
...
test('shortHash truncates without emptying', () => {
    ...
    assert.ok(
        ...
        'short hash should be prefix of full digest'
    );
    assert.equal(
        shortHash(hash, 0).length,
        1,
        'length is clamped to minimum of 1'
    );
});
...
test('pseudonymizeIncidentPointers hashes Discord IDs and preserves other fields', () => {
    ...
    assert.equal(
        hashed.traceId,
        pointers.traceId,
        'traceId should remain unchanged'
    );
    assert.equal(
        hashed.jumpUrl,
        pointers.jumpUrl,
    ...
    assert.equal(
        hashed.modelVersion,
        pointers.modelVersion,
        'modelVersion should remain unchanged'
    );

    assert.notEqual(
        hashed.guildId,
        pointers.guildId,
        'raw guildId should not persist'
    );
});
...
test('pseudonymizeActorId is idempotent and null-safe', () => {
    ...
    assert.equal(
        hashed,
        hashedAgain,
        'Should not double-hash existing digests'
    );
