    ...
    async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
        try {
            ...
            if (!process.env.GITHUB_WEBHOOK_SECRET) {
                ...
                res.end(
                    JSON.stringify({
                        message: 'Ignored: webhook not configured',
                    })
                );
            ...
            if (contentLengthHeader) {
                ...
                if (
                    ...
                    logRequest(
                        req,
                        res,
                        `webhook payload-too-large contentLength=${contentLength}`
                    );
                    return;
                }
            }
            ...
            try {
                payload = JSON.parse(body.toString()) as Record<
                    string,
                    unknown
                >;
            } catch (error) {
                logger.warn(
                    `Webhook received invalid JSON body: ${error instanceof Error ? error.message : String(error)}`
                );
                res.statusCode = 400;
                res.setHeader(
                    'Content-Type',
            ...
            if (repository.full_name !== expectedRepository) {
                ...
                res.end(
                    JSON.stringify({ message: 'Ignored: wrong repository' })
                );
            ...
            if (discussion.category?.name !== 'Blog') {
                ...
                res.end(
                    JSON.stringify({ message: 'Ignored: not Blog category' })
                );
            ...
            if (action !== 'created' && action !== 'edited') {
                ...
                res.end(
                    JSON.stringify({ message: 'Ignored: action not relevant' })
                );
            ...
            res.end(
                JSON.stringify({ success: true, postNumber: discussion.number })
            );
            logRequest(
                req,
                res,
                `webhook success postNumber=${discussion.number}`
            );
        } catch (error) {
            res.statusCode = 500;
            res.setHeader('Content-Type', 'application/json; charset=utf-8');
