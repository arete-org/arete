...
const renderScoreIcon = (row: ScoreRow): string => {
    ...
    if (row.key === 'evidence') {
        return `<svg id="trace-icon-evidence" x="${toSvgNumber(iconX)}" y="${toSvgNumber(iconY)}" width="${SCORE_ICON_SIZE}" height="${SCORE_ICON_SIZE}" viewBox="0 0 192 192" aria-hidden="true" focusable="false"><path d="${FINGERPRINT_ICON_PATH}" fill="${SCORE_ICON_COLOR}" stroke="${SCORE_ICON_COLOR}" stroke-width="4"/></svg>`;
    }

    return `<svg id="trace-icon-freshness" x="${toSvgNumber(iconX)}" y="${toSvgNumber(iconY)}" width="${SCORE_ICON_SIZE}" height="${SCORE_ICON_SIZE}" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path fill-rule="evenodd" clip-rule="evenodd" d="${CLOCK_ICON_PATH}" fill="${SCORE_ICON_COLOR}" stroke="${SCORE_ICON_COLOR}" stroke-width="0.5" /></svg>`;
};
