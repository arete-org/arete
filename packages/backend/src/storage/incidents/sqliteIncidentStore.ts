...
export class SqliteIncidentStore {
    ...
    private mapIncidentRow(row: Record<string, unknown>): IncidentRecord {
        return {
            id: row.id as number,
            shortId: row.short_id as string,
            status: row.status as IncidentStatus,
            tags: row.tags_json
                ? (JSON.parse(row.tags_json as string) as string[])
                : [],
            pointers: row.pointers_json
                ? (JSON.parse(row.pointers_json as string) as IncidentPointers)
                : {},
            remediationApplied: Boolean(row.remediation_applied),
            remediationNotes:
                (row.remediation_notes as string | null | undefined) ?? null,
    ...
    async createIncident(input: CreateIncidentInput): Promise<IncidentRecord> {
        ...
        incidentLogger.info(
            `Incident created (id=${id}, pointers=${shortPointerIds})`
        );
        const row = await this.withRetry(() =>
            this.getIncidentStatement.get(id)
        );
        return this.mapIncidentRow(row as Record<string, unknown>);
    }
    ...
    async appendAuditEvent(
        ...
        if (actorHash) {
            incidentLogger.info(
                `Audit event appended (incident=${incidentId}, actorHash=${shortHash(actorHash)})`
            );
        } else {
            incidentLogger.info(
                `Audit event appended (incident=${incidentId}, actorHash=none)`
            );
        }
