    ...
    const writeBlogPost = async (
        ...
        try {
            ...
            indexArray = indexArray.filter(
                (post) => post.number !== discussion.number
            );
            ...
            logger.error(
                `Error writing blog post: ${error instanceof Error ? error.message : String(error)}`
            );
            throw error;
        }
    };
    ...
    const readBlogIndex = async (): Promise<BlogPostIndexEntry[]> => {
        try {
            ...
            const indexContent = await fs.readFile(
                path.join(blogPostsDir, 'index.json'),
                'utf8'
            );
            const parsed = JSON.parse(indexContent);
            return Array.isArray(parsed)
                ? (parsed as BlogPostIndexEntry[])
                : [];
        } catch {
            return [];
        }
