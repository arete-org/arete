...
const METADATA_MARKER = '<RESPONSE_METADATA>';
...
/**
 * Extracts text and metadata from AI response that may contain <RESPONSE_METADATA> payload.
 */
function extractTextAndMetadata(
    ...
    try {
        ...
        logger.warn(
            `Failed to parse assistant metadata payload: ${error instanceof Error ? error.message : String(error)}`
        );
        return { normalizedText: conversationalPortion, metadata: null };
    }
}
