...
export class OpenAIService {
    ...
    public async reduceContext(
        ...
        const summarizerPrompt = renderPrompt(
        ...
        for (let i = 0; i < context.length; i++) {
            ...
            if (context[i].content.length < REDUCE_OVER_N_CHARCTERS) {
                continue;
            } else {
        ...
        if (messageIndexesToReduce.length > 0) {
            ...
            try {
                ...
                if (response.choices[0].message.content) {
                    ...
                    logger.debug(
                        `Summaries: ${JSON.stringify(response.choices[0].message.content)}`
                    );
                    ...
                    if (summaries.length !== messageIndexesToReduce.length) {
                        logger.warn(
                            `Number of summaries (${summaries.length}) does not match number of messages to summarize (${messageIndexesToReduce.length})`
                        );
                    }
                    ...
                    for (let i = 0; i < messageIndexesToReduce.length; i++) {
                        ...
                        const summaryContent =
                            summaries[i] ??
                            context[messageIndexesToReduce[i]].content;
                    ...
                    logger.debug(
                        `Reduced context: ${JSON.stringify(reducedContext)}`
                    );
                    ...
                    logger.debug(
                        `Estimated cost of reduction: ${formatUsd(reductionCost.totalCost)}`
                    );
                    if (this.costEstimator && response.usage) {
                        try {
