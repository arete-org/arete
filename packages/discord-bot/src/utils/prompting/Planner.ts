    ...
    parameters: {
        ...
        If 'action' is 'image', always use 'text'.`,
            ...
            reaction: {
                type: 'string',
                description:
                    "A string containing only emoji characters (no text). Required when action is 'react'. Example: 🤖👍",
            },
            imageRequest: {
                type: 'object',
                description:
                ...
                properties: {
                    ...
                    aspect_ratio: {
                        ...
                        description:
                            'Override aspect ratio for the generated image, only if explicitly requested (otherwise omit).',
                    },
                    background: {
                        type: 'string',
                        enum: ['auto', 'transparent', 'opaque'],
                        description:
                            'Override background mode for the generated image, only if explicitly requested (otherwise omit).',
                    },
                    output_format: {
                        type: 'string',
                        enum: ['png', 'webp', 'jpeg'],
                        description:
                            'Override output format, only if explicitly requested (otherwise omit).',
                    },
                    output_compression: {
                        type: 'integer',
                        minimum: 1,
                        maximum: 100,
                        description:
                            'Override compression quality (1-100% quality), only if explicitly requested (otherwise omit).',
                    },
                    quality: {
                        type: 'string',
                        enum: ['low', 'medium', 'high', 'auto'],
                        description:
                            'Override image quality, only if explicitly requested (otherwise omit).',
                    },
                    style: {
                        type: 'string',
                        description:
                            'Override style preset (e.g., natural, photorealistic, watercolor), only if explicitly requested (otherwise omit).',
                    },
                    allowPromptAdjustment: {
                        type: 'boolean',
                        description:
                            'Whether the model may adjust the prompt before rendering. Leave false unless the user explicitly asks for prompt improvements.',
                    },
                    followUpResponseId: {
                        type: 'string',
                        description:
                            'Existing response ID when requesting a variation.',
                    },
                },
                required: ['prompt'],
            },
            openaiOptions: {
                ...
                properties: {
                    reasoningEffort: {
                        ...
                        description:
                            "The level of reasoning to use, with 'low' being the default. 'minimal' is for very simple tasks, 'high' is for complex tasks and should be used sparingly.",
                    },
                    verbosity: {
                        type: 'string',
                        enum: ['low', 'medium', 'high'],
                        description:
                            "The level of verbosity to use. Prefer 'low' for casual conversation, and 'medium' for more detailed responses. Only use 'high' when explicitly asked to be verbose.",
                    },
                    tool_choice: {
                        type: 'object',
                        properties: {
                    ...
                    ttsOptions: {
                        ...
                        properties: {
                            ...
                            styleDegree: {
                                type: 'string',
                                enum: ['low', 'normal', 'high'],
                                description: 'Weight of speaking style.',
                            },
                            styleNote: {
                                type: 'string',
                                description:
                                    'Additional notes about the speaking style.',
                            },
                        },
                        required: [
                            'speed',
                        ...
                        items: {
                            ...
                            properties: {
                                type: {
                                    ...
                                    enum: [0, 1, 2, 3, 4, 5],
                                    description:
                                        'Activity type: 0 = Playing, 1 = Streaming, 2 = Listening, 3 = Watching, 4 = Custom (prefer this), 5 = Competing.',
                                },
                                name: {
                                    type: 'string',
                                    description:
                                        "The activity alone (e.g., chess) to be used with prefixed activity types (e.g., Playing chess, but without the 'Playing ' prefix). 24 characters max.",
                                },
                                state: {
                                    type: 'string',
                                    description:
                                        'The full activity string (e.g., Playing chess with Jordan). 30 characters max.',
                                },
                                url: {
                                    type: 'string',
                                    description:
                                        'Streaming URL (required if type = 1)',
                                },
                            },
...
export class Planner {
        ...
        try {
            ...
            const openaiResponse = await this.openaiService.generateResponse(
                ...
                [
                    ...
                    {
                        role: 'system',
                        content: `This planner was triggered because ${trigger}.`,
                    }, // The planner should know how it was triggered: Either a Discord direct reply/ping, or it decided to reply itself (e.g. a catchup event)
                    ...messages,
                ],
                {
                    ...PLANNING_OPTIONS,
            ...
            logger.debug(
                `Plan generated. Usage: ${JSON.stringify(response.usage)}`
            );
            ...
            if (funcCall?.arguments) {
                try {
                    ...
                    logger.debug(
                        `Validated plan: ${JSON.stringify(validatedPlan)}`
                    );
                    return validatedPlan;
                } catch {
                    logger.warn(
                        'Failed to parse plan arguments, using default'
                    );
                    return defaultPlan;
                }
            }
    ...
    private validatePlan(plan: Partial<Plan>): Plan {
        ...
        if (plan.imageRequest) {
            mergedPlan.imageRequest = this.normalizeImageRequest(
                plan.imageRequest
            );
        }
    ...
    private normalizeRiskTier(candidate: Partial<Plan>['riskTier']): RiskTier {
        ...
        if (candidate !== undefined) {
            logger.warn(
                `Planner returned unexpected risk tier "${candidate}", defaulting to ${DEFAULT_RISK_TIER}.`
            );
        }
    ...
    private normalizeImageRequest(
        request: Partial<ImagePlanRequest>
    ): ImagePlanRequest {
        const aspectRatioCandidate =
            (request as Record<string, unknown>).aspect_ratio ??
            request.aspectRatio;
        const aspectRatio = this.isValidAspectRatio(aspectRatioCandidate)
        ...
        const background =
            typeof request.background === 'string'
                ...
                : undefined;
        const style =
            typeof request.style === 'string' ? request.style : undefined;
        const quality = this.normalizeQuality(
            (request as Record<string, unknown>).quality ?? request.quality
        );

        const formatCandidate =
            (request as Record<string, unknown>).output_format ??
            request.outputFormat;
        const normalizedFormat =
            typeof formatCandidate === 'string'
                ? this.normalizeOutputFormat(formatCandidate)
                : undefined;

        const compressionCandidate =
            (request as Record<string, unknown>).output_compression ??
            request.outputCompression;
        const normalizedCompression =
            typeof compressionCandidate === 'number' ||
            typeof compressionCandidate === 'string'
                ? this.clampOutputCompression(compressionCandidate)
                : undefined;

        const followUpResponseId =
            typeof request.followUpResponseId === 'string' &&
        ...
        return {
            ...
            allowPromptAdjustment:
                request.allowPromptAdjustment !== undefined
    ...
    private isValidAspectRatio(
        value: unknown
    ): value is ImagePlanRequest['aspectRatio'] {
        return (
            value === 'auto' ||
            value === 'square' ||
            value === 'portrait' ||
    ...
    private normalizeQuality(candidate: unknown): ImageQualityType | undefined {
        ...
        if (normalized) {
            logger.warn(
                `Planner returned unsupported image quality "${candidate}", ignoring override.`
            );
        }
        return undefined;
    }
    ...
    private normalizeOutputFormat(
        ...
        if (normalized) {
            logger.warn(
                `Planner returned unsupported output format "${candidate}", ignoring override.`
            );
        }
        return undefined;
    }
