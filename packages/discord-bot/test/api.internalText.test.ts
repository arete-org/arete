/**
 * @description: Covers the Discord bot internal text-task API client wrapper.
 * @footnote-scope: test
 * @footnote-module: DiscordInternalTextApiTests
...
import { createInternalTextApi } from '../src/api/internalText.js';
...
const createImageDescriptionRequest = (
    overrides: Partial<PostInternalImageDescriptionTaskRequest> = {}
): PostInternalImageDescriptionTaskRequest => ({
...
test('runNewsTaskViaApi posts to /api/internal/text with trusted headers and returns parsed data', async () => {
    ...
    const api = createInternalTextApi(requestJson, {
...
test('runImageDescriptionTaskViaApi posts to /api/internal/text with trusted headers and returns parsed data', async () => {
    const request = createImageDescriptionRequest();
    let capturedEndpoint = '';
    let capturedHeaders: Record<string, string> | undefined;
    let capturedBody: unknown;
    ...
    const response = await api.runImageDescriptionTaskViaApi(request);

    assert.equal(capturedEndpoint, '/api/internal/text');
    assert.equal(capturedHeaders?.['X-Trace-Token'], 'trace-secret');
    assert.deepEqual(capturedBody, request);
    assert.equal(response.task, 'image_description');
    assert.match(response.result.description, /Screenshot of a policy update/);
});
